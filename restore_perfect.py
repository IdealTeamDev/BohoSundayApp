...
def apply_replace(content, args):
    ...
    # Extract the block of text from start_line to end_line
    block_lines = lines[start_line:end_line]
    block_text = '\n'.join(block_lines)
    
    if target in block_text:
        if allow_multi:
            new_block_text = block_text.replace(target, replacement)
        else:
        ...
        # If target not found exactly, try replacing \r\n with \n in target
        target_norm = target.replace('\r\n', '\n')
        if target_norm in block_text:
            if allow_multi:
                new_block_text = block_text.replace(target_norm, replacement)
            else:
            ...
            print(f"Warning: Target not found in specified range! Falling back to line replacement for {target.strip()[:30]}")
            # Fallback to pure line replacement if target is not found (which means target was wrong in the log?)
            lines = lines[:start_line] + replacement.split('\n') + lines[end_line:]
            return '\n'.join(lines)
...
print("Done restoring codebase perfectly!")
